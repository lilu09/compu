                                  ...
                                  , const   XMLSize_t       length)
{
    ...
    ((Element *)current)->content=c2string(chars);
}
...
{
    // No escapes are legal here called
	//fFormatter << XMLFormatter::NoEscapes << gEndElement << name << chCloseAngle;
    
	if(current)
	current= current->father;
	//test
	//cout<<"current: "<<current->tag<<endl;

/*
   const XMLSize_t len = XMLString::stringLen(name);
   char s[len+1];
   for(int i=0;i<len;++i)
   s[i]=(char)*(name+i);
   s[len]='\0';
...
{
    // The name has to be representable without any escapes called
	/*
	...
	*/
	...
	Element *n=new Element(a);
	 ...
	 for (XMLSize_t index = 0; index < len; index++)
	{
		 string name=c2string(attributes.getName(index));
		 string value=c2string(attributes.getValue(index));
...
string SAXPrintHandlers::c2string(const     XMLCh* const name)
{
    const XMLSize_t len = XMLString::stringLen(name);
   char s[len+1];
   for(int i=0;i<len;++i)
