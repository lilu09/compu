...
int xmlparse(std::string xmlFile, std::string ns_schema, Node *&tree)
{
	DEBUG_METHOD_ENTRY("xmlparse") // used for debugging purposes to trace problems...
    ...
    if(!inf)
    	error("Cannot open the XML file for parsing: " + xmlFile);
	else
		inf.close();
    ...
    //
	//char * descr = getenv("CT_PATH");
	//string path(descr);
	//path=path+"/";
    //string schema_path=path+"ComponentMetaData.xsd";
	//string ns_schema="http://www.peppher.eu/ComponentMetaData0.1 "+schema_path;

    SAXParser* parser = new SAXParser;
	parser->setValidationScheme(valScheme);
	parser->setDoNamespaces(true);
	parser->setDoSchema(true);
	parser->setExternalSchemaLocation(ns_schema.c_str());
    ...
    if(errorCode) {
    	warn("XML parser has errors while parsing "+xmlFile);
